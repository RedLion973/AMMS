...
def create_notice_types(app, created_models, verbosity, **kwargs):
    ...
    notification.create_notice_type("daily_report_generated", _("Daily Report Generated"), _("Daily report has been generated"))
...
def create_states(app, created_models, verbosity=1, **kwargs):
    ...
    for choice in settings.STEERING_STATES:
        f = open(os.path.join(settings.BASE_DIR, 'tmp/' + defaultfilters.slugify(choice[0]).replace('-','_') + '.png'))
        icon = File(f)
